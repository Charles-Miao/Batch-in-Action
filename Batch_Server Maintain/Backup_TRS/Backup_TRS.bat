...
::Delete other files(None Backup files), 21 days ago files will be delete in this server
...
for /f "delims=" %%i in ('dir /ad /b /s "E:\OOB"') do rd %%i 2>nul
for /f "delims=" %%i in ('dir /ad /b /s "E:\Temp"') do rd %%i 2>nul
set /a count=%count%+1
goto del_empty_dir 
:end_del_dir
echo ***************************************Delete empty folders***********************************************
